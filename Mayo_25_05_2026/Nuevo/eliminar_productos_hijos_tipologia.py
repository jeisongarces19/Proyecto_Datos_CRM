...
# Archivos de entrada y salida en la misma carpeta del script
INPUT_JSON = BASE_DIR / "productos-hijos-tipologia_Original.json"
OUTPUT_JSON = BASE_DIR / "productos-hijos-tipologia.json"
OUTPUT_ELIMINADOS_JSON = BASE_DIR / "productos-hijos-tipologia_eliminadas.json"
...
def cargar_codigos_a_eliminar(txt_path: Path) -> set:
    ...
    if not txt_path.exists():
        print(f"⚠️ No se encontró el archivo de códigos: {txt_path.resolve()}")
...
def cargar_json(path: Path):
    """
    Carga un JSON y devuelve una lista de objetos.
    """
    if not path.exists():
        print(f"❌ Archivo no encontrado: {path.resolve()}")
        return []
...
def filtrar_productos_hijos_tipologia(items, codigos_eliminar: set):
    """
    Filtra los elementos cuyo código esté en:
    item["producto"]["codigo"]

    Devuelve:
    - filtrados: elementos conservados
    - eliminados: elementos removidos
    """
    filtrados = []
    eliminados = []
...
def main():
    print("============================================================")
    print(" FILTRO DE PRODUCTOS HIJOS DE TIPOLOGÍA")
    print("============================================================")
    print(f"📁 Carpeta del script: {BASE_DIR}")
    print(f"📄 JSON de entrada: {INPUT_JSON.resolve()}")
    print(f"📄 TXT de códigos: {CODIGOS_TXT.resolve()}")
    print("")
    ...
    datos_filtrados, datos_eliminados = filtrar_productos_hijos_tipologia(
        datos,
        codigos_eliminar
    )

    guardar_json(datos_filtrados, OUTPUT_JSON)
    guardar_json(datos_eliminados, OUTPUT_ELIMINADOS_JSON)
    ...
    codigos_realmente_eliminados = {
        normalizar_codigo(item.get("producto", {}).get("codigo"))
        for item in datos_eliminados
        if isinstance(item, dict) and isinstance(item.get("producto"), dict)
    }
    ...
    if codigos_no_encontrados:
        archivo_no_encontrados = BASE_DIR / "codigos_no_encontrados_productos_hijos_tipologia.txt"
